...
// Uses the real app classes with an in-memory cookie store; requires the
// dev stack up and a dev account (dev@dev.dev / Enclavd2026!).
// Run: flutter pub get && dart run tool/verify_live.dart
// Exit 0 = the full login -> me -> feed -> next-page flow works.
...
Future<void> main() async {
  ...
  // In-process 1400x1400 q90 JPEG, bigger than app output, so the multipart
  // create path carries a camera-sized payload.
  ...
  // Pull-to-refresh: the newer-than delta must surface the new post against an older id.
  ...
  // 6b. Hashtags + entity decoding (hashtag page contract)
  // Content with an apostrophe (stored as &#039;), a URL and a unique hashtag;
  // the tag page must surface the post with a total.
  ...
  // NOTE: the target post may already be liked; first toggle flips the state, second restores it.
  final target = p0;
  ...
  // The roundtrip restored the original state, so the dev user is listed iff the post ended liked.
  check(
  ...
  // Real RealtimeService against the dev sidecar (Apache /ws proxy): token from
  // the jar, WS handshake, join frame, history ack.
