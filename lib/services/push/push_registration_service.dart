...
class PushRegistrationService {
  ...
  /// Late-bound to the CURRENT app container at call time (a client from
  /// the first cold-start container has an empty jar and would 401 every
  /// registration until the next login).
  final Future<ApiClient> Function() _clientFactory;

  /// Upserts the token: FCM registration token OR Unified Push endpoint
  /// URL, for [transport] ('fcm' | 'unifiedpush'). Extensionless api/v1
  /// path; the action rides in the JSON body.
