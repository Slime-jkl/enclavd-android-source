...
void main() {
  ...
  group('card ignite', () {
    ...
    testWidgets('the spent day explains itself and stays unlit',
      ...
      expect(find.byType(IgniteFlameOverlay), findsNothing);
    ...
    testWidgets('re-pressing an ignited post replays the flame',
        (tester) async {
      await pumpCard(
        tester,
      ...
      expect(find.byType(IgniteFlameOverlay), findsOneWidget,
          reason: 'a re-press replays the animation and sound, site parity');
      expect(find.text(_limitCopy), findsNothing);

      // The layer drops itself once the vortex has run.
  ...
  group('flame asset', () {
    testWidgets('the site vortex decodes from the bundled .lottie',
        (tester) async {
      // Decoded straight off the bundle, not through the overlay's app-wide
      // cache: the cache is a production optimisation and would make this
      // order-dependent (an earlier test can leave its load pending).
      await tester.runAsync(() async {
        final composition =
            await AssetLottie('assets/animations/flame-vortex.lottie').load();
        expect(composition.frameRate, 30,
            reason: 'the site file: 43 frames at 30fps');
        expect(composition.duration.inMilliseconds, 1433);
      });
    });

    testWidgets('the overlay mounts the decoded flame inside a card',
        (tester) async {
      await tester.pumpWidget(MaterialApp(
        theme: buildEnclavdTheme(),
        home: const Scaffold(
          body: SizedBox(
            ...
            height: 160,
            child:
                Stack(children: [Positioned.fill(child: IgniteFlameOverlay())]),
          ),
        ),
      ));
      await tester.pump(const Duration(milliseconds: 100));
  ...
  group('likers sheet', () {
    testWidgets('only the igniters carry the fire', (tester) async {
      ...
      expect(find.byType(IgniteFlameIcon), findsOneWidget,
          reason: 'one flame, on the igniter only');

      final flame = tester.widget<IgniteFlameIcon>(find.byType(IgniteFlameIcon));
      ...
      final flameBox = tester.getRect(find.byType(IgniteFlameIcon));
